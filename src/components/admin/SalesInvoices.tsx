...
import { adminUsersApi, salesApi, warrantyApi, categoriesApi, type AdminOrder, type StockProduct } from "@/lib/api";
...
import {
  ...
  AlertTriangle, PackageCheck, BarChart3, Boxes, Save, Loader2, TrendingUp, Plus, Trash2, MoreHorizontal, Shield, CreditCard, Tag, ScanBarcode, StickyNote, Truck
} from "lucide-react";
                ...
                <div className="p-4 space-y-3">
                  ...
                  <Button
                    ...
                    onClick={() => setSerialProduct({
                      ...
                      name: product.name,
                      icon_url: product.icon_url,
                      variants: product.variants.map(v => ({ id: v.id, combination: v.combination, sku: v.sku, stock_quantity: v.stock_quantity, price_adjustment: v.price_adjustment, display_color: v.display_color, variant_image: v.variant_image })),
                    })}
                  >
                    ...
                    <tr key={order.id} className={`group transition-colors hover:bg-muted/40 ${idx !== orders.length - 1 ? 'border-b border-border/50' : ''}`}>
                      ...
                      <td className="px-4 py-3.5 text-right">
                        <div className="flex items-center justify-end gap-0.5 opacity-70 group-hover:opacity-100 transition-opacity">
                          ...
                          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => printCustomerLabelDirect(order, order.customer?.address || order.user?.address || "Cambodia")} title="Print Label"><Tag className="w-3.5 h-3.5" /></Button>
                          <DropdownMenu>
                      ...
                      <div key={i} className="flex items-center justify-between bg-muted/50 rounded-lg p-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            {item.variant_label && <p className="text-[10px] text-muted-foreground leading-none mt-0.5">{item.variant_label}</p>}
...
<DialogContent className="sm:max-w-sm">
      ...
      {/* Print Customer Label — prints directly, no dialog */}
    </div>
  );
};
