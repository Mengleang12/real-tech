                      ...
                      {product.variants.map((v) => {
                        ...
                        return (
                          <div key={v.id} className={`inline-flex items-center gap-1 text-[11px] rounded-lg border transition-colors ${
                            ...
                            >
                              <span
                                className="w-2.5 h-2.5 rounded-full flex-shrink-0 border border-border/40"
                                style={v.display_color ? { backgroundColor: v.display_color } : undefined}
                                {...(!v.display_color ? { className: `w-1.5 h-1.5 rounded-full flex-shrink-0 ${v.is_active ? getStockColor(vStatus) : 'bg-muted-foreground/30'}` } : {})}
                              />
